...
Np = length(pdb(1,:));  % Number of paths
...
for ip = 1:Np
    ...
    QTx = calculateQ(NcTx(ip), pdb(:,ip), sigmaTx, delta_theta);
    ...
    % Calculation of approximate normalization coefficient(s)
    QRx = calculateQ(NcRx(ip), pdb(:,ip), sigmaRx, delta_theta);
...
function Q = calculateQ(Nc, pdb, sigma, delta_theta)
...
if Nc == 1
    ...
    pdb(pdb==-Inf) = 0;
    Q = zeros(1, Nc);
    exponentials = exp(-sqrt(2)*delta_theta./sigma);
    sigma(sigma==Inf) = 0;
    coeffs = sigma.*10.^(pdb/10);
    denominator = coeffs.*(1-exponentials);
    Q = coeffs.'/sum(denominator);
